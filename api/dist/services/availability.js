...
export function getPotentialSlots(date, restaurant) {
    console.log({ date });
...
export function getAvailability(restaurantId, sectorId, date, partySize) {
    ...
    const candidateTables = tables.filter(t => t.sectorId === sectorId &&
        t.minSize <= partySize &&
        partySize <= t.maxSize);
    const slots = [];
